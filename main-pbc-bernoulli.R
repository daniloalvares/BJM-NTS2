...
start_time1 <- Sys.time()
fitJS <- stan(file   = "stan/JS_bernoulli.stan", 
              data   = list(N=nrow(pbc2), n=n, y=as.numeric(pbc2$serBilir>2), times=pbc2$year,
                            Time=pbc2.id$years, status=pbc2.id$status2, 
                            x=as.numeric(pbc2.id$drug)-1, ID=rep(1:n,M),
                            K=K, xk=xk, wk=wk),
              init   = init_fun1,
              warmup = 1000,                 
              iter   = 2000,                
...
fitL <- stan(file   = "stan/LongTS_bernoulli.stan", 
             data   = list(N=nrow(pbc2), n=n, y=as.numeric(pbc2$serBilir>2), times=pbc2$year,
                           x=as.numeric(pbc2.id$drug)-1, ID=rep(1:n,M)),
             init   = init_fun21,
             warmup = 1000,                 
...
fitSTS <- stan(file   = "stan/STS_bernoulli.stan", 
               data   = list(n=n, Time=pbc2.id$years, status=pbc2.id$status2,
                             x=as.numeric(pbc2.id$drug)-1, 
...
fitNTS <- stan(file   = "stan/NTS_bernoulli.stan", 
               data   = list(N=nrow(pbc2), n=n, y=as.numeric(pbc2$serBilir>2), times=pbc2$year,
                             Time=pbc2.id$years, status=pbc2.id$status2,
                             x=as.numeric(pbc2.id$drug)-1, ID=rep(1:n,M),
