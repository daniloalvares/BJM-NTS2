...
// ------------------------------------------------------
{
   ...
   // Linear predictor
   invlogitmu = linear_predictor(x, times, ID, beta, bi);

   // Longitudinal Bernoulli log-likelihood
   target += bernoulli_logit_lpmf(y | invlogitmu);
}
// ------------------------------------------------------
...
// ------------------------------------------------------
{
  ...
  for(i in 1:n){
       // Hazard function
       haz[i] = phi * pow(Time[i], phi-1) * exp( gamma[1] + gamma[2] * x[i] + alpha * inv_logit(beta[1] + bi[i,1] + (beta[2] + bi[i,2])*Time[i] + beta[3] * x[i]) );

       // Hazard function evaluated at Gauss-Legendre quadrature integration points
       for(j in 1:K){
           cumHazK[i,j] = phi * pow(Time[i]/2*(xk[j]+1), phi-1) * exp( gamma[1] + gamma[2] * x[i] + alpha * inv_logit(beta[1] + bi[i,1] + (beta[2] + bi[i,2])*Time[i]/2*(xk[j]+1) + beta[3] * x[i]) );
       }
